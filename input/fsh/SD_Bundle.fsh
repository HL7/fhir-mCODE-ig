...
//* entry.resource.meta 1..1 MS
//* entry.resource.meta.profile 1..* MS  // each entry must have exactly one profile declared
* entry ^slicing.discriminator.type = #pattern
//* entry ^slicing.discriminator.path = "resource.$this.meta.profile"
...
* insert BundleSlice(primaryCancerCondition, 0, *, Primary Cancer Condition, Condition resource representing the Primary Cancer Condition., PrimaryCancerCondition)
...
* insert BundleSlice(cancerGenomicVariant, 0, *, Cancer Genomic Variant, Observation resource representing genomic variants., CancerGenomicVariant)
* insert BundleSlice(cancerGenomicsReport, 0, *, Cancer Genomics Report, DiagnosticReport resource representing cancer genomics reports., CancerGenomicsReport)
