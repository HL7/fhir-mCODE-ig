...
| Phase | A phase is a conceptual series of treatments that focus on one set of target volumes with the same modality and technique. A phase has one initial treatment plan that may be replaced by an adapted treatment plan during the phase. One or more related phases constitute a single course of treatment. | Phase is an abstraction from treatment plans to allow for continuous counting of phase fractions and dose tracking across multiple treatment plans. In the simplest case, a phase corresponds 1:1 to a treatment plan. Often phases within a course are sequential in time, but they may occur in parallel, overlapping, or in alternating temporal patterns. |
